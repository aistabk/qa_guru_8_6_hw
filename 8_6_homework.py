...
def test_dark_theme_by_time():
    ...
    is_dark_theme = (time(hour=0) <= current_time <= time(hour=5)) or \
                    (time(hour=22) <= current_time <= time(hour=23))
...
def function_attribute(func, *args):
    name_convert = func.__name__.replace("_", " ").title()
...
def open_browser(browser_name):
    actual_result = function_attribute(open_browser, browser_name)
    assert actual_result == "Open Browser [Chrome]"


def go_to_companyname_homepage(page_url):
    actual_result = function_attribute(go_to_companyname_homepage, page_url)
    assert actual_result == "Go To Companyname Homepage [https://companyname.com]"


def find_registration_button_on_login_page(page_url, button_text):
    actual_result = function_attribute(find_registration_button_on_login_page, page_url, button_text)
